...
from dqn_atari.agents.nstep_dqn_agent import calc_nstep_loss
from dqn_atari.envs.atari_wrappers import make_env
from dqn_atari.models.dqn import DQN
from dqn_atari.replay.nstep_buffer import NStepBuffer
...
def main():
    ...
    writer = SummaryWriter(comment="-" + args.env + "-nstep-dqn")
    print(net)

    buffer = NStepBuffer(N_STEPS, GAMMA, REPLAY_SIZE)
    ...
    while True:
        ...
        if reward is not None:
            ...
            if best_m_reward is None or best_m_reward < m_reward:
                torch.save(net.state_dict(), args.env + "-nstep_dqn-best_%.0f.dat" % m_reward)
        ...
        loss_t = calc_nstep_loss(batch, net, target_net, GAMMA_N, device)
        loss_t.backward()
        optimizer.step()
